...
for size_i = 1:length(samples_vec)
    ...
    tic
    res_temp = fct_cost_TDF_MC(omgn_vec_1, omgn_vec_2, samples_vec(size_i));
...
function dxdt = fct_inside_real_system(x,omgn)
    u_mod = 1;
    dxdt(1,1) = x(2);
    dxdt(2,1) = -omgn^2*x(1) + omgn^2*u_mod;
end

function J = fct_cost_TDF_MC(fct_omgn_vec_1, fct_omgn_vec_2, N)
...
% t_2 for given pair (omega_n, omega_m)
    ...
    parfor sample_i = 1:N
        sample_i
        [TT1, YY_MC_1] = ode45(@(t,x) fct_inside_real_system(x,fct_omgn_vec_1(sample_i)), t1_vec, x0_MC_1, options);
        x0_MC_2 = YY_MC_1(end, :);
        [TT2, YY_MC_2] = ode45(@(t,x) fct_inside_real_system(x,fct_omgn_vec_2(sample_i)), t2_vec, x0_MC_2, options);
